 ...
 */
class TaskMap {
    constructor(task, doTimes, fulfilled) {
     ...
     */
    static dec(obj, key) {
        return new TaskMap(parseInt(Cryp.decrypt(obj.task, key)), parseInt(Cryp.decrypt(obj.doTimes, key)), Cryp.decrypt(obj.fulfilled, key) == "T" ? true : false);
    }
    /**
     * 是否完成任务
     * @returns 结果
...
const tasks = [
    ...
    new Task("双重加密，双重保护", "设置二级锁。你可以在一个文件夹中点击右上角的“加密”图标，这样就可以给这个文件夹加设二级锁。", Folder.root(), 500),
];
const nameplates = [
    new Nameplate("熵锁锚核", "冰封的叹息在鲸骨纹路里结晶", "能冻结错误时间线的扩散，将量子病毒困在绝对零度的牢笼", `<p><strong>《东海科学报》报道</strong></p>
        ...
        <p>今夜我值最后一班岗。凌晨三点，显示屏突然跳出一串1987年的日期代码。锚核在防护罩里缓慢旋转，像极了老家那台总在除夕夜停摆的座钟。冰霜顺着电缆爬进控制台时，我听见母亲的声音。她二十年前就去世了，在东海辐射监测站做保洁员的母亲。"孩子，把锚核送去第七区......"话音未落，应急灯突然投射出布满整个天花板的鲸骨纹路。那纹路间闪烁着ES-014的荧光编号，像星空般寂静。</p>`, `<p>我是保安老张，在海洋研究院看了二十年大门。那晚陈院长冲进值班室时，手里攥着的玉米馍还冒着冷气，馍皮上凝着冰碴子。他说自动贩卖机突然吐出1987年的粮票食物时，我就知道要出大事。</p>
        ...
        <p>现在每当我经过第七区仓库，怀里的老怀表就会停摆。表盘玻璃上映出的永远是1987年12月7日，父亲跳入反应堆冷却池的最后一刻。</p>`, 2),
    new Nameplate("极光棱镜", "碎镜中打捞起的第13次日落", "折射数据污折射被篡改的历史轨迹，在星空投射真相的坐标染轨迹", `<p><strong>《气象观察》期刊</strong></p>
        ...
        <p>现在每到月圆之夜，灯塔玻璃就会映出极光路径。昨晚光路尽头指向我的烫伤疤——那是我七岁时打翻父亲实验日志留下的。火焰吞噬了"人体锚点"四个字，却在皮肤上烙下ES-014的焦痕。</p>`, 3),
    new Nameplate("雷蚀龙芯", "囚禁在青铜里的亿万次心跳", "释放生物电脉冲，烧毁伪造的记忆回路", `<p><strong>《电力安全通报》</strong></p>
        ...
        <p>立方体裂开时，里面掉出半张焚烧过的《量子焚化协议》，签署日期是2174年12月7日。现在我的左臂布满龙鳞状疤痕，每当雷声响起，疤痕就会在墙上投出倒计时：距离锚点归位还有214天。</p>`, 4),
    new Nameplate("时溯沙漏", "吞下晨昏线的漩涡吐出骨灰", "用死者的时间尘埃预演未来三十天的可能性", `<p><strong>《突发事件预警》</strong></p>
        ...
        <p>现在我的安检仪每天凌晨三点会扫描出异常物品：有时是1987年的实验报告，有时是沾着海水的婴儿襁褓。昨天扫出张泛黄的照片，母亲抱着我站在第七区铁门前，她白大褂上别着蔚蓝计划的徽章。</p>`, 5),
    new Nameplate("星火燧芯", "陨石胎衣中啼哭的盗火者", "焚烧时间悖论产生的数据残渣，净化时空连续体", `<p><strong>《环保监察快报》</strong></p>
        ...
        <p>现在每当燧芯发热，我就能听见妻子撕心裂肺的呼喊。她在火中说："去海底找鸣匣，那是修正时间的最后机会"。</p>`, 6),
    new Nameplate("血棘菌脉", "铁锈花蕊中绽放的旧日战争", "向篡改者植入记忆病毒，使其被自己的谎言反噬", `<p><strong>《生物安全警报》</strong></p>
        ...
        `, 7),
    new Nameplate("鲸墟鸣匣", "溺亡的巴别塔将在大海深处复活", "重启所有被切断的因果链，让平行时空共振归一", `<p><strong>《海洋考古重大发现》</strong></p>
        ...
        <p>海风吹散晨雾时，第七区的废料井长出大片蓝花。我摘下朵花别在胸口，花瓣背面刻着：时间线已修正，ES-014归档。</p>`, 8)
];
const levelMap = [
    -1,
    0,
...
function goHome(token) {
    ...
    main === null || main === void 0 ? void 0 : main.scrollTo(pagePos.home);
}
