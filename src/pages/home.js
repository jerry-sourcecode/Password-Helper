...
class Permission {
     ...
     * @returns 结果
     */
    canAddPwd(alreadyHave) {
        if (this.pwdNum == -1)
            return true;
        else
            return this.pwdNum >= alreadyHave + 1;
    }
    /**
     * 是否可以添加文件夹
     ...
     * @returns 结果
     */
    canAddFolder(alreadyHave) {
        if (this.folderNum == -1)
            return true;
        else
            return this.folderNum >= alreadyHave + 1;
    }
}
/**
 * 用户组类
...
let isHasNewUserGroup = false;
/**
 * 刷新待办事项
 */
function updateNeedTODOTasks() {
    NEEDTODO = [];
